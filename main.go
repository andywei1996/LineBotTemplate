...
// 				}
				if message.Text == "幹" {
					_, err = bot.ReplyMessage(event.ReplyToken, linebot.NewTextMessage(" 牟利洗烈幹三小!")).Do();
				}
				else{
					_, err = bot.ReplyMessage(event.ReplyToken, linebot.NewTextMessage(message.Text+" OK!")).Do();
				}
				if err != nil {
					log.Print(err)
				}
